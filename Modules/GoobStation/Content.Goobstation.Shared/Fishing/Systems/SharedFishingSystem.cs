...
public abstract partial class SharedFishingSystem : EntitySystem
{
    ...
    protected virtual void CalculateFightingTimings(Entity<ActiveFisherComponent> fisher, ActiveFishingSpotComponent activeSpotComp) { }

    /// <summary>
    /// Server-side only, spawns a fish and throws it to our player!
    /// </summary>
    protected virtual void ThrowFishReward(EntProtoId fishId, EntityUid fishSpot, EntityUid target) { }

    private void SetupFishingFloat(Entity<FishingRodComponent> fishingRod, EntityUid player, EntityCoordinates target)
    {
        var (uid, component) = fishingRod;
        var targetCoords = Xform.ToMapCoordinates(target);
        if (!Xform.TryGetMapOrGridCoordinates(player, out var playerCoords))
            return;

        var fishFloat = PredictedSpawnAtPosition(component.FloatPrototype, playerCoords.Value);
        component.FishingLure = fishFloat;
        DirtyField(uid, component, nameof(FishingRodComponent.FishingLure));

        // Calculate throw direction
        var direction = targetCoords.Position - playerCoords.Value.Position;
        ...
        DirtyField(fishFloat, fishLureComp, nameof(FishingLureComponent.FishingRod));
    ...
    private void OnPullFloat(Entity<FishingRodComponent> ent, ref PullFishingLureActionEvent args)
    {
        ...
        if (lureComp.AttachedEntity != null && Exists(lureComp.AttachedEntity))
        {
            ...
            var rand = new Random(HashCode.Combine((int) Timing.CurTick.Value, GetNetEntity(ent.Owner))); // evil random prediction hack
