...
public sealed partial class ResearchSystem
{
    ...
    {
        if (!Resolve(serverUid, ref database) || !PrototypeManager.TryIndex<TechnologyPrototype>(technologyId, out var technology) || database.RevealedTechnologies.Contains(technology.ID))
            return;
    ...
    public void UnlockTechnology(EntityUid serverUid, string technologyId, EntityUid? user, TechnologyDatabaseComponent? database = null)
    {
        if (!Resolve(serverUid, ref database) || !PrototypeManager.TryIndex<TechnologyPrototype>(technologyId, out var technology) || database.ResearchedTechnologies.Contains(technology.ID))
            return;
